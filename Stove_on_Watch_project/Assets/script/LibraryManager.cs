    ...
    */
    ...
    public abst_Plr_action return_action() {
        ...
        return Activator.CreateInstance(
            Type.GetType(action_list[code].Substring(0, action_list[code].Length - 3))
            ) as abst_Plr_action;
    }
    ...
    public abst_enemy return_enemy() {
        ...
        return Activator.CreateInstance(
            Type.GetType(enemy_list[code].Substring(0, enemy_list[code].Length - 3))
            ) as abst_enemy;
    }

    public abst_event return_event(bool is_first) {
        List<string> temp = is_first ? event_first_list : event_second_list;
        code = GameManager.g.ran.xoshiro_range(event_both_list.Count + temp.Count);
        if (code < event_both_list.Count) {
            return Activator.CreateInstance(
                Type.GetType(event_both_list[code].Substring(0, event_both_list[code].Length - 3))
                ) as abst_event;
        } else {
            code -= event_both_list.Count;
            return Activator.CreateInstance(
                Type.GetType(temp[code].Substring(0, temp[code].Length - 3))
                ) as abst_event;
        }
    }
    #endregion return
